...
function NFTCard({ imageUrl, title, creator, price, likes, comments, trending }: NFTProps) {
      ...
      <div className="relative overflow-hidden rounded-xl glow-hover bg-navy/30 backdrop-blur-sm">
        ...
        >
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <h3 className="font-poppins font-bold text-lg truncate mb-1 glow-text">{title}</h3>
            ...
            <div className="flex items-center gap-4">
              <button
                className={`flex items-center gap-1 transition-colors duration-200
                  ${isLiked ? 'text-pink-500' : 'text-gray-300 hover:text-pink-500'}`}
                onClick={handleLike}
              >
                <Heart size={18} fill={isLiked ? 'currentColor' : 'none'}
        ...
        <div className="md:hidden p-3">
          <h3 className="font-poppins font-bold text-base truncate">{title}</h3>
...
const nfts = [
  ...
  {
    imageUrl: "https://plus.unsplash.com/premium_photo-1690522330973-021425184c2d",
    title: "Digital Metropolis",
    creator: "NFTMaster",
    price: 1.8,
    likes: 892,
